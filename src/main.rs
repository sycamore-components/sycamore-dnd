...
use sycadrop::{create_draggable, create_droppable, DropEffect};
...
fn Item<'cx, G: Html>(
    ...
    let node = create_draggable::<G, i32>(cx)
        .data(item.id)
        .allowed_effect(DropEffect::Move)
        .dragging_class("dragging")
        .build();

    let node = create_droppable::<G, i32>(cx)
        .node_ref(node)
        .on_drop({
            move |dragged_id: i32| {
                log!("Swapping {} and {}", item.id, dragged_id);
